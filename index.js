...
class DevToMaster {
	...
	branchName() {
		return `merge_${this.sourceBranch}_to_${this.targetBranch}_${Date.now()}`;
	}
	...
	prBody() {
		return `This pull request merges the latest changes from \`${this.sourceBranch}\` branch into \`${this.targetBranch}\`.\n\n**Automatically generated by GitHub Action**`;
	}
};
				...
				git.remote(["set-url", "origin", url])
					...
					.catch(error => {
						if (error.message.includes("CONFLICT")) {
							console.log(
								"Merge conflict detected, aborting merge.");
							git.merge(["--abort"]).then(() => {
								bad("Merge conflict occurred. Aborting.");
							});
						} else {
							bad(error);
						}
...
const processPullRequest = info => {
	...
	octokit.rest.pulls.list(info.common).then(response => {
		const existingPR
			= response.data.find(i => i.head.ref === updater.branchName());
